...
S=$M/dflash2/backend-master-status.txt; : > "$S"
exec >>$M/dflash2/backend-master.log 2>&1
arm(){ local label="$1" bin="$2"; shift 2
  ...
  env "$@" "$bin" -m $M/Qwen3.8-27B-Q4_K_S.gguf -md $M/mtp-Qwen3.8-27B-Q4_0.gguf -ngld 99 \
    ...
    -c 32768 -b 4096 -ub 2048 -np 1 --host 127.0.0.1 --port $PORT > $M/dflash2/bm-$label.log 2>&1 &
  local SRV=$!
  ...
  grep -aq "Qwen3.8-27B-Q4_K_S.gguf" $M/dflash2/bm-$label.log || { echo "  $label ABORT wrong model" >>"$S"; kill -9 $SRV; return 1; }
...
PY
  grep -aoE "draft acceptance = [0-9.]+" $M/dflash2/bm-$label.log | tail -1 | sed "s/^/  $label /" >> "$S"
  kill -9 $SRV 2>/dev/null; sleep 4
}
arm vk-master  /home/jrouth/llama.cpp-master/build-vk/bin/llama-server     _=_
arm hip-master /home/jrouth/llama.cpp-master/build-hip/bin/llama-server LD_LIBRARY_PATH=$R/lib
echo DONE >> "$S"
